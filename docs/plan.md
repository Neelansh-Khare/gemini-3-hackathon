...
1. **True Multi-Agent Deliberation**: 
...
1. **Reversibility (Undo)**: 
    - Implement a "Rollback" mechanism. Since the system is "append-only by default," this would involve deleting the newly created entities or reverting the specific append.
2. **Enhanced Diff Preview**: 
    - Move beyond JSON payload previews. Show a "Visual Diff" (e.g., "Calendar: Add 'Team Lunch' at 12pm") that is more legible for non-technical users.
